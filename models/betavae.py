...
class BetaVAE(tf.keras.Model):
    ...
    def elbo(self, x, **kwargs):
        ...
        mean_z, logvar_z, z_sample, x_pred = self.forward(x)

        logpx_z = compute_log_bernouli_pdf(x_pred, x)
    ...
    def train_step(self, x, optimizers, **kwargs):
        with tf.GradientTape() as tape:
            elbo, logpx_z, kl_divergence = self.elbo(x, **kwargs)
    ...
    def encode(self, x):
        mean_z, logvar_z = tf.split(self.encoder(x), num_or_size_splits=2, axis=-1)
    ...
    def generate(self, z=None, num_generated_images=15, target=None):
        if z is None:
    ...
    def average_kl_divergence(self, x):
        mean_z, logvar_z = self.encode(x)
